  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f5f5', fontFamily: "'Be Vietnam Pro', sans-serif" }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px', height: 64, display: 'flex', alignItems: 'center', gap: 20 }}>
          ...
          <Link href="/" style={{ color: 'white', fontWeight: 800, fontSize: 20, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 10, whiteSpace: 'nowrap', flexShrink: 0 }}>
            <span style={{
              ...
              fontSize: 22, flexShrink: 0, border: '1px solid rgba(255,255,255,0.3)',
            }}>{siteEmoji}</span>
            <div>
          ...
          <form method="GET" action="/" style={{ flex: 1, maxWidth: 560, position: 'relative' }}>
            <input
              ...
              style={{
                ...
                outline: 'none', boxShadow: '0 2px 12px rgba(0,0,0,0.12)',
            ...
            <button type="submit" style={{
              ...
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              boxShadow: `0 2px 8px ${primary}66`, transition: 'transform 0.15s',
            }}>🔍</button>
          </form>
        </div>
        ...
        <div style={{ background: 'rgba(0,0,0,0.14)', borderTop: '1px solid rgba(255,255,255,0.12)' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px', display: 'flex', gap: 2, overflowX: 'auto', scrollbarWidth: 'none' }}>
            {[{ name: 'Tất cả', slug: undefined }, ...categories].map(c => {
              ...
              return (
                <Link key={c.slug || 'all'} href={c.slug ? `/?cat=${c.slug}` : '/'} className="cat-tab" style={{
                  padding: '10px 18px',
      ...
      <div className="trust-bar" style={{
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', justifyContent: 'center', gap: 28, flexWrap: 'wrap' }}>
          {[shippingText, guaranteeText, returnText].map((t, i) => (
            <span key={i} style={{ fontSize: 12, color: '#555', fontWeight: 500, display: 'flex', alignItems: 'center', gap: 4 }}>{t}</span>
          ))}
        </div>
      </div>
      ...
      {bannerShow && !catSlug && !query && (
        <div style={{
          background: `linear-gradient(135deg, ${primary}ee 0%, ${primary} 50%, ${primary}cc 100%)`,
          ...
          <div style={{ position: 'absolute', top: 20, left: '20%', width: 60, height: 60, borderRadius: '50%', background: 'rgba(255,255,255,0.04)', pointerEvents: 'none' }} />

          <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative' }}>
            <div className="banner-title" style={{ fontSize: 32, fontWeight: 800, fontFamily: 'Nunito, sans-serif', marginBottom: 10, textShadow: '0 2px 12px rgba(0,0,0,0.15)', letterSpacing: '-0.5px' }}>
              {bannerTitle}
            </div>
            <div className="banner-sub" style={{ fontSize: 15, opacity: 0.9, maxWidth: 480, margin: '0 auto 20px' }}>
              {bannerSubtitle}
            </div>
            <Link href="#products" className="banner-sub" style={{
      ...
      <footer style={{ background: footerColor, color: '#aaa', padding: '48px 20px 28px', marginTop: 48 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ marginBottom: 16 }}>
            <div style={{ color: 'white', fontWeight: 800, fontSize: 20, fontFamily: 'Nunito, sans-serif', marginBottom: 6 }}>
              {siteEmoji} {siteName}
            </div>
            <div style={{ fontSize: 13, maxWidth: 400, margin: '0 auto', lineHeight: 1.7, color: 'rgba(255,255,255,0.45)' }}>
              {footerText}
            </div>
          </div>
          ...
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)', paddingTop: 20, fontSize: 12, color: 'rgba(255,255,255,0.3)' }}>
            {footerCopyright}
          </div>
        </div>
      </footer>
