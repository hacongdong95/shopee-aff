...
function getStockPercent(id: number): number {
  return Math.floor(seededRandom(id * 23) * 40 + 15)
}
  ...
  return (
    <Link href={`/san-pham/${product.slug}`} style={{ textDecoration: 'none', display: 'block', height: '100%' }}>
      <div className="product-card" style={{
        ...
        onMouseLeave={e => { const el = e.currentTarget as HTMLDivElement; el.style.transform = ''; el.style.boxShadow = '0 2px 10px rgba(0,0,0,0.07)' }}
      >
        {/* Image */}
        ...
        <div style={{ padding: '12px 13px 14px', flex: 1, display: 'flex', flexDirection: 'column', gap: 7 }}>
          ...
          {/* ── Giá ngang hàng ── */}
          <div style={{ background: '#fff8f6', borderRadius: 8, padding: '8px 10px' }}>
            {product.oldPrice && product.oldPrice > product.price ? (
              <>
                {/* Hàng 1: giá bán trái + giá gốc phải */}
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
                  <span style={{ color: 'var(--primary)', fontWeight: 800, fontSize: 18, lineHeight: 1 }}>
                    {product.price.toLocaleString('vi-VN')}<span style={{ fontSize: 11, fontWeight: 600 }}>đ</span>
                  </span>
                  <span style={{ color: '#bbb', fontSize: 11, textDecoration: 'line-through', whiteSpace: 'nowrap' }}>
                    {product.oldPrice.toLocaleString('vi-VN')}đ
                  </span>
                </div>
                {/* Hàng 2: tiết kiệm trái + % phải */}
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 5 }}>
                  <span style={{ fontSize: 11, color: '#059669', fontWeight: 700 }}>
                    💰 Tiết kiệm {(product.oldPrice - product.price).toLocaleString('vi-VN')}đ
                  </span>
                  <span style={{ background: 'var(--primary)', color: 'white', fontSize: 11, fontWeight: 800, padding: '2px 7px', borderRadius: 20 }}>
                    -{discount}%
                  </span>
                </div>
              </>
          ...
          <div>
            ...
            <div style={{ height: 4, background: '#f0f0f0', borderRadius: 4, overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${100 - stockPct}%`, background: stockPct < 25 ? 'linear-gradient(90deg,#e74c3c,#c0392b)' : 'linear-gradient(90deg,#f39c12,#e67e22)', borderRadius: 4 }} />
            </div>
          </div>
