        ...
        {showReviews && (
          <div style={{ background:'white', borderRadius:12, boxShadow:'0 2px 8px rgba(0,0,0,0.06)', overflow:'hidden', marginBottom:16 }}>
            <div onClick={() => setOpenReviews(o => !o)}
              ...
              <span style={{ fontSize:20, color:primary, transition:'transform 0.2s', display:'inline-block', transform: openReviews ? 'rotate(180deg)' : 'rotate(0deg)' }}>â–¾</span>
            </div>
        ...
        {related.length>0 && (
          <div style={{ background:'white', borderRadius:12, boxShadow:'0 2px 8px rgba(0,0,0,0.06)', overflow:'hidden' }}>
            ...
            <div style={{ padding:'12px 12px 16px' }}>
              <style>{`@media(max-width:900px){.related-grid{grid-template-columns:repeat(4,1fr)!important}}@media(max-width:600px){.related-grid{grid-template-columns:repeat(3,1fr)!important}}`}</style>
              <div className="related-grid" style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:1, border:'1px solid #f0f0f0', borderRadius:8, overflow:'hidden' }}>
                {related.map((p, idx) => {
